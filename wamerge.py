...
def help():
    ...
    ** Github: https://github.com/B16f00t
    ...
    Usage: python wamerge.py -h (for help)
    """
...
def merge(db_path, db_name):
    ...
    if os.path.isdir(db_path):
        ...
        if not list_dbs:
            print ("[i] Database not found in the path")
            exit()
        ...
        for filename in list_dbs:
            if (filename != list_dbs[0]) and (filename != db_name):
                ...
                print "\n   [-] " + db_name + " --> " + str(len(ids_message_write)) + " messages, " + str(len(ids_chatlist_write)) + " chats, " + str(len(ids_quote_write)) + " replies, " + str(len(ids_thumb_write)) + " thumbnails"
